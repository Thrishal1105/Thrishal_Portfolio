...
import { EXPERIENCE, EDUCATION, SKILLS_DATA, SOFT_SKILLS } from '../../constants';
            ...
            {EXPERIENCE.map((job) => (
              <div key={job.id} className="relative flex flex-col md:flex-row gap-4 md:gap-8 group">
                ...
                <div className="md:w-[160px] flex-shrink-0 md:text-right md:pr-8 pt-0.5">
                   <span className="font-heading font-bold text-white block text-lg">{job.period.split(' ')[0]}</span>
                   <span className="text-neon-green text-sm font-semibold mb-1 block">{job.period.replace(/^\d{4}\s*/, '')}</span>
                   <span className="text-gray-400 text-sm block md:mt-2">{job.company}</span>
                </div>

                {/* Right Column: Role & Desc */}
                <div className="flex-1 pb-8 md:pb-0 relative pl-6 md:pl-0 border-l border-gray-700 md:border-l-0">
                  {/* Timeline visual (Mobile) */}
                   ...
                   <h4 className="text-xl font-bold text-white mb-1 leading-tight">{job.role}</h4>
                   <p className="text-gray-400 text-sm leading-relaxed mt-2">{job.description}</p>
                </div>
              </div>
            ))}
          </div>
